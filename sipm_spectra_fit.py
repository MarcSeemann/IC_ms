...
if 'y' in plot_option:
    ...
    for ich in range(len(specsL)): # Loop over all channels
        ...
        next_plot = input('press enter to move to next fit, or press s to save current plot')
        if 's' in next_plot: # Option to save a given plot
            plt.savefig('FitSiPMCh'+str(ich)+'.png')
        plt.clf()
        plt.close()
